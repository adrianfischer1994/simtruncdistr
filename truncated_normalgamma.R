...
clusterEvalQ(cl,{
  ...
  #       return(dnorm(x,mean=mu,sd=sqrt(sigma))*dgamma(y,shape=alpha,rate=beta))
  #     }
  ...
  #calculation of the normalizing constant for domain through points a,b,c
  norm_c <- function(mu,sigma,alpha,beta,a,b,c) {
    
    inner_int <- function(y) {
      inner_func <- function(x) {
        return(dnorm(a[1]+y*(b[1]-a[1])+x*(c[1]-a[1]),mean=mu,sd=sqrt(sigma))*dgamma(a[2]+y*(b[2]-a[2])+x*(c[2]-a[2]),shape=alpha,rate=beta))
      }
      inner_func <- Vectorize(inner_func)
      return(integrate(inner_func,lower=0,upper=1-y)$value)
    }
    inner_int <- Vectorize(inner_int)
    
    return(abs((b[1]-a[1])*(c[2]-a[2])-(c[1]-a[1])*(b[2]-a[2]))*integrate(inner_int,lower=0,upper=1)$value)
  }
...
sim <- function(param) {
  ...
  #functions for Stein estimation for domain through 3 points a,b,c
  #IMPORTANT: the lines (a,b) and (b,c) bound the domain from above and the line (c,a) from below
  a=c(-1,0)
  b=c(-0.5,7)
  c=c(5,0.05)
  kappa <- function(x) {
    return(((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*
             ((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*
              ((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2]))
  }
  dkappa <- function(x) {
    erg1 <-((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*(c[2]-a[2])/(c[1]-a[1])
    erg2 <-((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*(b[2]-c[2])/(b[1]-c[1])*((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])
    erg3 <-(a[2]-b[2])/(a[1]-b[1])*((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])
    
    erg4 <-((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])
    erg5 <-((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])
    erg6 <-((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])
    
    return(c(erg1+erg2+erg3,-erg4-erg5-erg6))
  }
  ...
  #functions for Score matching for domain through 3 points a,b,c
  distthreep <- function(x) {
    dist1 <- abs((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])/norm(c((a[2]-b[2])/(a[1]-b[1]),-1),type = "2")
    dist2 <- abs((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])/norm(c((b[2]-c[2])/(b[1]-c[1]),-1),type = "2")
    dist3 <- abs((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])/norm(c((c[2]-a[2])/(c[1]-a[1]),-1),type = "2")
    distmin <- min(dist1,dist2,dist3)
    
    if(distmin==dist1) {
      ddistmin1 <- sign((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*((a[2]-b[2])/(a[1]-b[1]))/norm(c((a[2]-b[2])/(a[1]-b[1]),-1),type = "2")
      ddistmin2 <- sign((a[2]-b[2])/(a[1]-b[1])*x[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])-x[2])*(-1)/norm(c((a[2]-b[2])/(a[1]-b[1]),-1),type = "2")
    }
    
    if(distmin==dist2) {
      ddistmin1 <- sign((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*((b[2]-c[2])/(b[1]-c[1]))/norm(c((b[2]-c[2])/(b[1]-c[1]),-1),type = "2")
      ddistmin2 <- sign((b[2]-c[2])/(b[1]-c[1])*x[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])-x[2])*(-1)/norm(c((b[2]-c[2])/(b[1]-c[1]),-1),type = "2")
    }
    
    if(distmin==dist3) {
      ddistmin1 <- sign((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])*((c[2]-a[2])/(c[1]-a[1]))/norm(c((c[2]-a[2])/(c[1]-a[1]),-1),type = "2")
      ddistmin2 <- sign((c[2]-a[2])/(c[1]-a[1])*x[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])-x[2])*(-1)/norm(c((c[2]-a[2])/(c[1]-a[1]),-1),type = "2")
    }
    
    return(c(distmin,ddistmin1,ddistmin2))
  }
  ...
  for(i in 1:m) {
    ...
    # while(n_tmp<(n+1)) {
    #   x_tmp <- c(rnorm(1,mu,sqrt(sigma)),rgamma(1,shape=alpha,rate=beta))
    ...
    #Sampling for truncation wrt domain through 3 points a,b,c
    while(n_tmp<(n+1)) {
      x_tmp <- c(rnorm(1,mu,sqrt(sigma)),rgamma(1,shape=alpha,rate=beta))
      if( ((a[2]-b[2])/(a[1]-b[1])*x_tmp[1]+(a[1]*b[2]-a[2]*b[1])/(a[1]-b[1])>x_tmp[2]) &
          ((b[2]-c[2])/(b[1]-c[1])*x_tmp[1]+(b[1]*c[2]-b[2]*c[1])/(b[1]-c[1])>x_tmp[2]) &
          ((c[2]-a[2])/(c[1]-a[1])*x_tmp[1]+(c[1]*a[2]-c[2]*a[1])/(c[1]-a[1])<x_tmp[2]) ) {
        X[n_tmp,] <- x_tmp
        n_tmp <- n_tmp+1
      }
    }
    ...
    tryCatch({
      ...
      #log-likelihood for circles
      # ll <- function(par) {
      #   C <- norm_c(par[1],par[2],par[3],par[4],a,r)
      #   return(n*log(C)-sum(log(dnorm(X[,1],mean=par[1],sd=sqrt(par[2]))))-sum(log(dgamma(X[,2],shape=par[3],rate=par[4]))))
      # }
      
      #log-likelihood for domain thorugh points a,b,c
      ll <- function(par) {
        C <- norm_c(par[1],par[2],par[3],par[4],a,b,c)
        return(n*log(C)-sum(log(dnorm(X[,1],mean=par[1],sd=sqrt(par[2]))))-sum(log(dgamma(X[,2],shape=par[3],rate=par[4]))))
      }
    ...
    tryCatch({
      ...
      #This is the numericl optimisation code with respect to the circle, but explicit estimators are available
      # smf <- function(par) {
      #   func_temp <- function(x) (r-norm(x-a,type="2"))*((x[1]-par[1])^2/par[2]^2+((par[3]-1)/x[2]-par[4])^2) + 2*(r-norm(x-a,type="2"))*(-1/par[2]-(par[3]-1)/x[2]^2) + 
      #                                     2*((x[1]-a[1])*((x[1]-par[1])/par[2])-(x[2]-a[2])*((par[3]-1)/x[2]-par[4]))/norm(x-a,type="2")
      #   return(mean(apply(X,1,func_temp)))
      # }
      # SMparam <- optim(c(0,1,1,1),smf)$par
      
      #These are the function with respect to the circle
      ...
      # 
      # func_temp <- function(x) -(r-norm(x-a,type="2"))*2/x[2]
      ...
      # 
      # func_temp <- function(x) -(r-norm(x-a,type="2"))*2*1/x[2]^2 - 2*(r-norm(x-a,type="2"))*1/x[2]^2 - 2*(x[2]-a[2])/norm(x-a,type="2")*1/x[2]
      # M9 <- mean(apply(X,1,func_temp))
      # 
      # func_temp <- function(x) (r-norm(x-a,type="2"))*2*1/x[2] + 2*(x[2]-a[2])/norm(x-a,type="2")
      # M10 <- mean(apply(X,1,func_temp))
      
      #These are the function with respect to the domain of three points
      func_temp <- function(x) distthreep(x)[1]
      M1 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) -2*distthreep(x)[1]*x[1]
      M2 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) distthreep(x)[1]*x[1]^2
      M3 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) distthreep(x)[1]/x[2]^2
      M4 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) distthreep(x)[1]
      M5 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) -distthreep(x)[1]*2/x[2]
      M6 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) -2*distthreep(x)[1]-distthreep(x)[2]*x[1]
      M7 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) distthreep(x)[2]
      M8 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) -distthreep(x)[1]*2*1/x[2]^2 - 2*distthreep(x)[1]*1/x[2]^2 + distthreep(x)[3]*1/x[2]
      M9 <- mean(apply(X,1,func_temp))
      
      func_temp <- function(x) distthreep(x)[1]*2*1/x[2] - distthreep(x)[3]
      M10 <- mean(apply(X,1,func_temp))
    ...
    tryCatch({
      ...
      STalpha <- (N1*N2-N3*N4)/(N3*N6-N5*N1)
      STbeta <- (N6*N2-N5*N4)/(N3*N6-N5*N1)
...
param <- list(c(1,2,3,4),c(0.5,1,4,5),c(0,1,1,1),c(0,0.1,0.5,3),c(0.2,0.3,0.1,1),c(0,1.5,3,0.5),c(0,0.4,3,3))
